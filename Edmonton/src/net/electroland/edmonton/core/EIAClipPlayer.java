...
public class EIAClipPlayer {
	...
	/*	
	 *  lookAhead is an arbitrary amount to look ahead for the next light
	 *  to anticipate walker movement
	 *  negative to move the animation left or "forward"
	 */
	...
	public void localTrill4Down(double x) {
		...
		double x1 = (int)findNearestLight(x+lookAhead,true);
		double x2 = findNearestLight(x1+barOff,true);
		double x3 = findNearestLight(x2+barOff,true);
		double x4 = findNearestLight(x3+barOff,true);
		int barWidth = 3;
	...
	public void twoNoteChord(double x){
		...
		int barWidth = 3;
		int of = 1; //offset to hit lights
		double x1 = findNearestLight(x+lookAhead,true);
		double x2 = findNearestLight(x1+barOff,true);
		//logger.info("twoNoteChord@ " + x + " where x1=" + x1 + " and x2=" + x2);

		Content chord1 = new SolidColorContent(Color.WHITE);
		Content chord2 = new SolidColorContent(Color.WHITE);
		Clip stab1 = live.addClip(chord1, (int)x1-of,0,barWidth,16, 1.0);
		Clip stab2 = live.addClip(chord2, (int)x2-of,0,barWidth,16, 1.0,168);
		//fade out
