...
public class EIAClipPlayer {
	...
	public void introLocalTrill4Up(double x) {
		...
		int trill2 = anim.startClip("simpleClip2", new Rectangle((int)x-2,0,2,16), 0.1);
		int trill3 = anim.startClip("simpleClip2", new Rectangle((int)x,0,2,16), 0.2); 
		int trill4 = anim.startClip("simpleClip2", new Rectangle((int)x+2,0,2,16), 0.3); 
		
		//now pop in trills2-4 on time
		anim.queueClipChange(trill2, null, null, 1.0, 0, 500, false); //popin 161
		anim.queueClipChange(trill3, null, null, 1.0, 0, 1500, false); //popin 307
		anim.queueClipChange(trill4, null, null, 1.0, 0, 3000, false); //popin 495
 
		
		//fade em all out
		anim.queueClipChange(trill1, null, null, 0.0, 500, 5000, true); //fadeout
		anim.queueClipChange(trill2, null, null, 0.0, 500, 5000, true); //fadeout
		anim.queueClipChange(trill3, null, null, 0.0, 500, 5000, true); //fadeout
		anim.queueClipChange(trill4, null, null, 0.0, 500, 5000, true); //fadeout
