...
public class Metaballs3 extends GothamPApplet {
    ...
    public void drawELUContent() {
        ...
        fill(color(128, 0, 255),127); //fill with purple
...
*/
    ...
    public void handle(GridData srcData) {
        if (gridData == null){
            ...
            synchronized(gridData){
                ...
                srcData = rotate(srcData, -90, new Point(grid.width / 2, grid.height / 2));
    ...
    public GridData subset(GridData in, Rectangle boundary){
        byte[] target = new byte[boundary.width * boundary.height];
        for (int y = 0; y < boundary.height; y++){
            System.arraycopy(in.data, ((y + boundary.y - 1) * in.height) + (boundary.x - 1), target, y * boundary.width, boundary.width);
        }
        in.data  = target;
    ...
    public GridData rotate(GridData in, float degrees, Point center){
    	/**
    ...
    public GridData flipHorizontal(GridData in){
