...
public class SensorPanelAnalog extends JPanel implements MouseMotionListener { // change mouselistener to mousemotionlistener to allow smooth drags
    ...
    //constructor
    public SensorPanelAnalog (int w, int h) {
    ...
    public void paint2SensorsAnalog(byte v1, byte v2){
        ...
        for (int r=0; r<rows; r++){
            ...
            gci.setColor(new Color(v1,0,0));
            gci.fillRect(c*boxWidth+margin+c*margin/2,r*boxWidth+margin+r*margin/2,boxWidth,boxWidth);
            //draw outline
            gci.setColor(new Color(255,255,255));
            gci.drawRect(c*boxWidth+margin+c*margin/2,r*boxWidth+margin+r*margin/2,boxWidth,boxWidth);
            gci.setFont(afont);
            gci.drawString("left = " + v1, c*boxWidth+margin+c*margin/2+boxWidth/2-24, (int)(r*boxWidth+margin+r*margin/2+boxWidth*1.1));
            
            c = 1;
            //value = v2;
            gci.setColor(new Color(v2,0,0));
            gci.fillRect(c*boxWidth+margin+c*margin/2,r*boxWidth+margin+r*margin/2,boxWidth,boxWidth);
            //draw outline
            gci.setColor(new Color(255,255,255));
            gci.drawRect(c*boxWidth+margin+c*margin/2,r*boxWidth+margin+r*margin/2,boxWidth,boxWidth);
            gci.setFont(afont);
            gci.drawString("right = " + v2, c*boxWidth+margin+c*margin/2+boxWidth/2-24, (int)(r*boxWidth+margin+r*margin/2+boxWidth*1.1));
