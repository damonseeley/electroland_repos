...
AudioMeter {
	...
	*new { arg index, target, addAction, point, label;
	...
	init { arg argindex, argtarget, argaddAction, argpoint, arglabel;
		...
		w = JSCWindow(label, rect, resizable: false);
		w.front;
		...
		index.do( { arg ix, i;
			...
			JSCStaticText(w, Rect( (i*30) + 10, 5, 25, 15))
				.string_(ix.asString).stringColor_(Color.black).align_(\center);
				
			clip.add(JSCButton(w, Rect( (i*30) + 10, 25, 25, 10)));
			...
			meter.add(JSCRangeSlider(w, Rect( (i*30) + 10, 40, 25, 160)));
			meter[i].knobColor = Color.black;
			//setting rangeSlider background this way with SwingOSC complains. Commenting.
			...
			peak.add(JSCNumberBox(w, Rect( (i*30) + 10, 205, 25, 15)));
			//setting the font param of a JSCNumberBox doesn't understand a Font instance.
			//commenting for now. What type does this expect?
