		...
		try {
			...
			while (running){
				...
				if (runner.custom.isSelected()){
					...
					if (runner.ascending.isSelected()){
						...
						if (ctr%2 == 0){
							...
							// DS ADDED THIS TO SET LOW VALUE
							for (int i = 2; i < packetSize - 1; i++){
								buffer[i] = (byte)3;
							}
				...
				for (int i = 0; i < targets.length; i++){
					...
					if (!runner.stepThroughRecipients.isSelected() ||
						isBroadcastTime ||
						(runner.stepThroughRecipients.isSelected() 
						 && ctr%targets.length == i)){
						...
						if (runner.logSends.isSelected() && !isBroadcastTime)
							System.out.println(targets[i] + "\tPacket sent=     " + toSend);
						if (runner.logOffsets.isSelected() && isBroadcastTime)
							System.out.println(targets[i] + "\tOffset sent=     " + toSend);
