...
//-----------------------------------------------------------------------------
int classifyPoint( Vec3f *vPoint, Plane *pPlane )
{
    Vec3f vDirection = pPlane->m_vPoint - *vPoint;

    float fResult = Vec3f::dotProduct( vDirection,  pPlane->m_vNormal );
...
CParticleSystem::CParticleSystem()
{
    ...
    m_fLifeCycle       = 1.0f;
    m_fSize            = 1.0f;
    m_clrColor         = Vec3f(1.0f,1.0f,1.0f);
    m_vPosition        = Vec3f(0.0f,0.0f,0.0f);
    m_vVelocity        = Vec3f(0.0f,0.0f,0.0f);
    m_vGravity         = Vec3f(0.0f,0.0f,0.0f);
    m_vWind            = Vec3f(0.0f,0.0f,0.0f);
...
// Desc: 
//-----------------------------------------------------------------------------
void CParticleSystem::SetCollisionPlane( Vec3f vPlaneNormal, Vec3f vPoint, 
    ...
    while( *ppParticle )
    {
        ...
        else
        {
            ...
            //-----------------------------------------------------------------
                        ...
                        Vec3f Vn = Vec3f::dotProduct( pPlane->m_vNormal, 
                                                            pParticle->m_vCurVel ) * pPlane->m_vNormal;

                        Vec3f Vt = pParticle->m_vCurVel - Vn;
                        Vec3f Vp = Vt - Kr * Vn;
                    ...
                    {
                        pParticle->m_vCurPos = vOldPosition;
                        pParticle->m_vCurVel = Vec3f(0.0f,0.0f,0.0f);
                    }
                }
    ...
    if( m_fCurrentTime - m_fLastUpdate > m_fReleaseInterval )
    {
        ...
        for( DWORD i = 0; i < m_dwNumToRelease; ++i )
        {
            ...
            if( m_dwActiveCount < m_dwMaxParticles )
            {
                ...
                {
                    Vec3f vRandomVec = getRandomVector();
...
void CParticleSystem::Render( void )
{
    ...
    if( m_renderMethod == RENDER_METHOD_MANUAL_CPU_BILLBOARDS )
    {
        ...
        Vec3f vRight( mat[0], mat[4], mat[8] );
        Vec3f vUp( mat[1], mat[5], mat[9] );
        Vec3f vPoint0;
        Vec3f vPoint1;
        Vec3f vPoint2;
        Vec3f vPoint3;
    ...
    if( m_renderMethod == RENDER_METHOD_CG_VERTEX_SHADER )
    {
        ...
        Vec3f vRight( mat[0], mat[4], mat[8] );
        Vec3f vUp( mat[1], mat[5], mat[9] );
        Vec3f vCenter( 0.0f, 0.0f, 0.0f );

        Vec3f vPoint0 = vCenter + (-vRight - vUp);
        Vec3f vPoint1 = vCenter + ( vRight - vUp);
        Vec3f vPoint2 = vCenter + ( vRight + vUp);
        Vec3f vPoint3 = vCenter + (-vRight + vUp);
