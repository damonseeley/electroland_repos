...
public class PersonTrackerVirtualChannel_v2 extends VirtualChannel {

    private static Logger logger = Logger.getLogger(PersonTrackerVirtualChannel_v2.class);
    ...
    public void configure(ParameterMap params) {
        ...
        detectionHoldoffFiltParams.put("waitToFallBelowThreshAfterMaxHoldoff", params.getDefault("waitToFallBelowThreshAfterMaxHoldoff", "true"));
        ...
        personPresentHoldoffFiltParams.put("holdoffLenMs",params.getDefault("personPresentHoldoffLenMs", "1500.0")); 
        personPresentHoldoffFiltParams.put("maxHoldoffLenMs",params.getDefault("personPresentMaxHoldoffLenMs", "20000.0")); 
    ...
    public Value processInputs(ValueSet inputValues) {
        ...
        if (personPresent) {
            ...
            // Update person-present hold-off timer
...
//                System.out.println("CH " + id + " - DETECTOR RE-ARMED AT " + (((double)System.currentTimeMillis() - startTimeMs)/1000));
        ...
        // Otherwise, the detector is armed, check if a new event has occurred
            ...
            //    Check if we should declare an event because one of the channels has clipped
                ...
                int i = 0;
                while ( i < 2 && (output.getValue() == PeopleIOWatcher.PersonEventCodes.NO_EVENT) ) {
